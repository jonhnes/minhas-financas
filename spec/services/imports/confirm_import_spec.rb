...
RSpec.describe Imports::ConfirmImport do
  ...
  it "reuses an existing open Bradesco statement when the final statement for the same period is confirmed" do
    user = create(:user)
    ...
    final_category = create(:category, user: user, name: "Categoria final")
    ...
    confirmed_statement = described_class.new(import: final_import).call

    expect(confirmed_statement.id).to eq(statement.id)
    expect(confirmed_statement.reload.document_kind).to eq("final_statement")
    expect(confirmed_statement.total_amount_cents).to eq(3_475)
    expect(confirmed_statement.metadata["import_id"]).to eq(final_import.id)
    expect(final_import.reload.statement).to eq(statement)
    expect(final_import).to be_confirmed
    ...
    expect(matched_item.reload.linked_transaction).to eq(existing_transaction)
  ...
  it "still rejects a final Bradesco statement when the same period is already finalized" do
    ...
    create(:import_item, import: import_record, category: category, metadata: { "provider_key" => "bradesco_pdf" })
    create(:statement,
      credit_card: credit_card,
      period_start: Date.new(2026, 3, 1),
      period_end: Date.new(2026, 3, 31),
